...
my $pat   = RE 'string::delimited';
my $pat_k = RE 'string::delimited', -Keep => 1;
...
foreach my $delim (q {"}, q {'}, q {`}) {
    ...
    $test -> match (qq {${delim}Escape the \\${delim}! ${delim}},
                    [[delimited  => qq {${delim}Escape the \\${delim}! } .
                                    qq {${delim}}],
                     [odelimiter => $delim],
                     [string     => qq {Escape the \\${delim}! }],
                     [cdelimiter => $delim]],
